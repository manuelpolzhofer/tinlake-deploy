...
export BORROWER_DEPLOYER=$(dapp create "src/borrower/deployer.sol:BorrowerDeployer" $ROOT_CONTRACT $TITLE_FAB $SHELF_FAB $PILE_FAB $COLLECTOR_FAB $FEED_FAB $TINLAKE_CURRENCY '"Tinlake Loan Token"' '"TLNFT"' $DISCOUNT_RATE)
[[ ! -z "$ETHERSCAN_API_KEY" ]] && dapp verify-contract --async "src/borrower/deployer.sol:BorrowerDeployer" $BORROWER_DEPLOYER $ROOT_CONTRACT $TITLE_FAB $SHELF_FAB $PILE_FAB $COLLECTOR_FAB $FEED_FAB $TINLAKE_CURRENCY '"Tinlake Loan Token"' '"TLNFT"' $DISCOUNT_RATE
...
[[ ! -z "$ETHERSCAN_API_KEY" ]] && dapp verify-contract --async 'lib/tinlake-title/src/title.sol:Title' $TITLE '"Tinlake Loan Token"' '"TLNFT"'

message "deploy pile contract"
seth send $BORROWER_DEPLOYER 'deployPile()'
export PILE="$(seth call $BORROWER_DEPLOYER 'pile()(address)')"
[[ ! -z "$ETHERSCAN_API_KEY" ]] && dapp verify-contract --async 'src/borrower/pile.sol:Pile' $PILE
...
[[ ! -z "$ETHERSCAN_API_KEY" ]] && dapp verify-contract --async 'src/borrower/feed/navfeed.sol:NAVFeed' $FEED
...
[[ ! -z "$ETHERSCAN_API_KEY" ]] && dapp verify-contract --async 'src/borrower/shelf.sol:Shelf' $SHELF $TINLAKE_CURRENCY $TITLE $PILE $FEED
...
[[ ! -z "$ETHERSCAN_API_KEY" ]] && dapp verify-contract --async 'src/borrower/collect/collector.sol:Collector' $COLLECTOR $SHELF $PILE $FEED
